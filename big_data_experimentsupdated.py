...
user = (
    ...
    .agg(
        ...
        missing_share=("active", lambda x: x.isna().mean()),
         days_active=("active", lambda x: (x == 1).sum())
    )
)
...
logging.info("Multiple outcomes + BH/FDR correction")
real_outcome = "converted"
np.random.seed(123)
real_outcome = "post_clicks"
np.random.seed(123)
is_ios = (user["platform"] == "ios").astype(float)
is_android = (user["platform"] == "android").astype(float)
user["placebo_1"] =  0.30 * np.log1p(user["baseline_activity"]) + 0.20 * user["pre_metric"] + 0.10 * is_ios     + np.random.normal(0, 1, size=len(user))
user["placebo_2"] = -0.10 * np.log1p(user["baseline_activity"]) + 0.40 * user["pre_metric"] + 0.05 * is_android + np.random.normal(0, 1, size=len(user))
user["placebo_3"] =  0.25 * user["pre_metric"]                 + 0.15 * is_ios              - 0.08 * is_android + np.random.normal(0, 1, size=len(user))
user["placebo_4"] =  0.50 * np.log1p(user["baseline_activity"])                                  + np.random.normal(0, 1, size=len(user))
user["placebo_5"] =  0.20 * np.log1p(user["baseline_activity"]) - 0.20 * user["pre_metric"]      + np.random.normal(0, 1, size=len(user))
user["placebo_6"] =  0.10 * user["pre_metric"] + 0.30 * (user["block"].astype(float) / user["block"].max())     + np.random.normal(0, 1, size=len(user))
user["placebo_7"] = -0.15 * user["pre_metric"] + 0.10 * is_ios  + 0.10 * is_android                            + np.random.normal(0, 1, size=len(user))
user["placebo_8"] =  0.05 * np.log1p(user["baseline_activity"]) + np.random.normal(0, 1, size=len(user))
user["placebo_9"] =  0.35 * user["pre_metric"]                 + np.random.normal(0, 1, size=len(user))

outcomes = [real_outcome] + [f"placebo_{i}" for i in range(1, 10)]

rows = []
for y in outcomes:
    fit = smf.ols(
        f"{y} ~ treat + baseline_activity + pre_metric + C(block)",
        data=user
    ).fit(cov_type="cluster", cov_kwds={"groups": user["cluster_id"]})

    tau_hat = fit.params["treat"]
    pval = fit.pvalues["treat"]

    rows.append({"outcome": y, "tau_hat": tau_hat, "p_value": pval})

results = pd.DataFrame(rows)
reject, p_bh, _, _ = multipletests(results["p_value"].to_numpy(), alpha=0.05, method="fdr_bh")

results["p_value_bh"] = p_bh
results["significant_q05"] = reject.astype(int)
results.to_csv("outputs/tables/multiple_testing.csv", index=False)

logging.info("Saved: outputs/tables/multiple_testing.csv")
...
fit_retained.summary2().tables[1].to_csv("outputs/tables/regression_retained_any.csv")
